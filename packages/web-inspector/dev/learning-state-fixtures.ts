...
export const LEARNING_SCREENSHOT_STATES = [
  ...APPROVED_LEARNING_STATES,
  ...BEHAVIOR_LEARNING_STATES,
] as const;

export type LearningScreenshotState =
  (typeof LEARNING_SCREENSHOT_STATES)[number];
...
function links(container = false) {
  ...
  return {
    learning,
    candidates: container ? `${learning}&tab=candidates` : null,
